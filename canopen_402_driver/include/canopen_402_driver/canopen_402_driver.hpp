...
public:
  explicit MotionControllerDriver(const rclcpp::NodeOptions& options) : ProxyDriver(options)
  {
    intialised = false;
  }
  ...
  void run()
  {
    if (!intialised)
    {
      ...
      timer_ = this->create_wall_timer(std::chrono::milliseconds(period_ms_),
                                       std::bind(&MotionControllerDriver::run, this), timer_group);
    }
  ...
  bool command_mode_switch(uint16_t mode);
