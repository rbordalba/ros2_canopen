...
void DeviceContainerNode::add_node_to_executor(const std::string& driver_name, const uint8_t node_id,
                                               const std::string& node_name)
{
  if (auto exec = executor_.lock())
  {
    ...
    auto node_instance =
        std::static_pointer_cast<ros2_canopen::DriverInterface>(node_wrappers_[node_id].get_node_instance());

    active_drivers_.insert({ node_name, std::make_pair(node_id, driver_name) });

    RCLCPP_INFO(this->get_logger(), "Added node of type %s with name \"%s\" for node_id %hhu to executor.",
                driver_name.c_str(), node_name.c_str(), node_id);
  }
  else
  {
...
void DeviceContainerNode::remove_node_from_executor(const std::string& driver_name, const uint8_t node_id,
                                                    const std::string& node_name)
{
  ...
  {
    RCLCPP_ERROR(this->get_logger(), "Failed to remove %s of type %s from executor", node_name.c_str(),
                 driver_name.c_str());
  }
}
...
bool DeviceContainerNode::load_component(std::string& package_name, std::string& driver_name, uint8_t node_id,
...
std::map<uint32_t, std::string> DeviceContainerNode::list_components()
{
  ...
  {
    components[wrapper.first] = wrapper.second.get_node_base_interface()->get_fully_qualified_name();
  }
  return components;
}
...
bool DeviceContainerNode::init_devices_from_config()
{
  ...
  for (auto it = devices.begin(); it != devices.end(); it++)
  {
    if (it->find("master") == std::string::npos)
    {
      ...
      RCLCPP_INFO(this->get_logger(), "Found Driver %s with lazy_load %s", driver_name.value().c_str(),
                  enable_lazy_load.value() ? "true" : "false");
...
bool DeviceContainerNode::init(const std::string& can_interface_name, const std::string& master_config,
...
void DeviceContainerNode::on_load_node(const std::shared_ptr<rmw_request_id_t> request_header,
...
void DeviceContainerNode::on_unload_node(const std::shared_ptr<rmw_request_id_t> request_header,
                                         const std::shared_ptr<UnloadNode::Request> request,
                                         std::shared_ptr<UnloadNode::Response> response)
{
}

void DeviceContainerNode::on_list_nodes(const std::shared_ptr<rmw_request_id_t> request_header,
