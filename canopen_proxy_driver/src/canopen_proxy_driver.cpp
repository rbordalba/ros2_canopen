...
void ProxyDriver::on_nmt(canopen::NmtState nmt_state)
{
  ...
  RCLCPP_INFO(this->get_logger(), "Slave %hhu: Switched NMT state to %s", this->driver->get_id(), message.data.c_str());
...
{
  RCLCPP_INFO(this->get_logger(), "Slave %hhu: Sent PDO index %hu, subindex %hhu, data %x", this->driver->get_id(),
              d.index_, d.subindex_, d.data_);
...
void ProxyDriver::on_nmt_state_reset(const std_srvs::srv::Trigger::Request::SharedPtr request,
...
void ProxyDriver::on_nmt_state_start(const std_srvs::srv::Trigger::Request::SharedPtr request,
                                     std_srvs::srv::Trigger::Response::SharedPtr response)
{
...
void ProxyDriver::on_sdo_read(const canopen_interfaces::srv::CORead::Request::SharedPtr request,
...
void ProxyDriver::on_sdo_write(const canopen_interfaces::srv::COWrite::Request::SharedPtr request,
